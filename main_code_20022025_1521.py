...
class VAEEvaluator:
    ...
    def mutual_information(latent):
        return torch.mean(latent * torch.log(latent + 1e-8))  # Avoid log(0)

    @staticmethod
    def total_correlation(latent):
        marginal_entropy = torch.mean(torch.log(torch.mean(torch.exp(latent), dim=0)))
        joint_entropy = torch.mean(torch.log(torch.exp(latent).mean(dim=0)))
...
class ConvVAE(nn.Module):
    ...
    def train_model(
        ...
        for epoch in range(num_epochs):
            ...
            if val_loader is not None:
                val_loss = ConvVAE.validate_model(model, val_loader, device)
                ...
                if val_loss < best_val_loss:
                    ...
                    epochs_no_improve = 0
                    print(f"    > Validation loss improved. Best model updated.")
                else:
                    epochs_no_improve += 1
        ...
        if best_model_wts is not None:
            ...
            best_model_path = os.path.join(ConvVAE.MODEL_DIR, model_name)
    ...
    def validate_model(model, val_loader, device):
        ...
        with torch.no_grad():
            for batch_data in val_loader:
                ...
                loss = ConvVAE.loss_function(batch_data, recon, mu, logvar)
                val_loss += loss.item()
        return val_loss / len(val_loader.dataset) if len(val_loader.dataset) > 0 else 0.0
        ...
        with torch.no_grad():
            for batch_data in data_loader:
                ...
                for i in range(batch_data.size(0)):
                    ...
                    tc_score = evaluator.total_correlation(mu).item()
    ...
    @staticmethod
    def load_model(model_path, device, latent_dim=DEFAULT_LATENT_DIM, input_height=DEFAULT_RESIZE_HEIGHT, input_width=DEFAULT_RESIZE_WIDTH):
        """
        Loads a saved VAE model from disk.
        """
        model = ConvVAE(input_height=input_height, input_width=input_width, latent_dim=latent_dim)
